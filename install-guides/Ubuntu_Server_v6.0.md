<h1>Cloud Native Core v6.0 - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>

This document describes how to setup the NVIDIA Cloud Native Core collection on a single or multiple NVIDIA Certified Systems. Cloud Native Core can be configured to create a single node Kubernetes cluster or to create/add additional worker nodes to join an existing cluster.

Cloud Native Core v5.0 includes:
- Ubuntu 20.04.3 LTS
- Containerd 1.6.0
...
- Upgraded to Containerd 1.6.0
- Upgraded to Kubernetes 1.23.2
- Upgraded to Helm 3.8.0
- Upgraded to NVIDIA GPU Operator 1.9.1 with NVIDIA Data Center GPU R510 Driver
...
Please note that Cloud Native Core is validated only on systems with the default kernel (not HWE).


### Installing the Ubuntu Operating System
These instructions require installing Ubuntu Server LTS 20.04.3. Ubuntu Server can be downloaded [here](http://cdimage.ubuntu.com/releases/20.04.3/release/).
...
```
kubectl set env daemonset/calico-node -n kube-system IP_AUTODETECTION_METHOD=interface=ens\*,eth\*,enc\*
```
...
Execute the following command to download and install Helm 3.8.0: 
...
Refer to the Helm 3.8.0 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information.
...
```
 helm install --version 1.9.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.version=510.47.03 --wait --generate-name
```
...
```
 helm install --version 1.9.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator  --set driver.rdma.enabled=true --set driver.version=510.47.03 --wait --generate-name
```
...
```
 helm install --version 1.9.1 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.rdma.enabled=true,driver.rdma.useHostMofed=true --set driver.version=510.47.03 --wait --generate-name 
...
NAME            	NAMESPACE       	REVISION	UPDATED                                	STATUS  	CHART                 	APP VERSION
network-operator	network-operator	1       	2022-03-03 17:09:04.665593336 +0000 UTC	deployed	network-operator-1.1.0	v1.1.0
