<h1>Cloud Native Core v4.2 (formely EGX Stack 4.2) - Install Guide for Ubuntu Server x86-64</h1>
<h2>Introduction</h2>

This document describes how to set up NVIDIA Cloud Native Core v4.2 on single or multiple servers to deploy AI applications via Helm charts from NVIDIA NGC. Cloud Native Core v4.2 will include:
...
Execute the following command to download and install Helm 3.7.0: 
...
Refer to the Helm 3.7.0 [release notes](https://github.com/helm/helm/releases) and the [Installing Helm guide](https://helm.sh/docs/using_helm/#installing-helm) for more information.
...
```
 helm install --version 1.9.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --wait --generate-name
```
...
```
 helm install --version 1.9.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator  --set driver.rdma.enabled=true --wait --generate-name
```
...
```
 helm install --version 1.9.0 --create-namespace --namespace gpu-operator-resources nvidia/gpu-operator --set driver.rdma.enabled=true,driver.rdma.useHostMofed=true --wait --generate-name 
...
gpu-operator-1606173805 default         1               2021-12-03 20:23:28.063421701 +0000 UTC deployed        gpu-operator-1.9.0      1.9.0 
...
NAME            	NAMESPACE       	REVISION	UPDATED                                	STATUS  	CHART                 	APP VERSION
network-operator	network-operator	1       	2021-12-04 17:09:04.665593336 +0000 UTC	deployed	network-operator-1.0.0	v1.0.0
